...
var ImageMapper = (function (_Component) {
	...
	function ImageMapper(props) {
		...
		["drawrect", "drawcircle", "drawpoly", "initCanvas", "renderPrefilledAreas", "renderOtherAreas"].forEach(function (f) {
		...
		this.watchedProps = ["active", "fillColor", "height", "imgWidth", "lineWidth", "src", "strokeColor", "width", "multiple"];
	}
	...
	_createClass(ImageMapper, [{
		...
		value: function renderAreas() {
			...
			return this.state.map.areas.map(function (area, index) {
				var scaledCoords = _this6.scaleCoords(area.coords);
				var center = _this6.computeCenter(area);
				var extendedArea = _extends({}, area, { scaledCoords: scaledCoords, center: center });
				return _react2["default"].createElement("area", {
					key: area._id || index,
					shape: area.shape,
					coords: scaledCoords.join(","),
					onMouseEnter: _this6.hoverOn.bind(_this6, extendedArea, index),
					onMouseLeave: _this6.hoverOff.bind(_this6, extendedArea, index),
					onMouseMove: _this6.mouseMove.bind(_this6, extendedArea, index),
					onClick: _this6.click.bind(_this6, extendedArea, index),
					href: area.href
				});
			});
		}
