...
var ImageMapper = (function (_Component) {
	...
	function ImageMapper(props) {
		...
		["drawrect", "drawcircle", "drawpoly", "initCanvas", "renderPrefilledAreas", "renderOtherAreas", "highlightAreas"].forEach(function (f) {
		...
		this.watchedProps = ["active", "fillColor", "height", "imgWidth", "lineWidth", "src", "strokeColor", "width", "highlight", "multiple", "map", "highlightColor"];
	}
	...
	_createClass(ImageMapper, [{
		...
		key: "renderOtherAreas",
		value: function renderOtherAreas(area) {
			var _this5 = this;

			var areas = this.state.map.areas.filter(function (a) {
				return a.name === area.name;
			});
			areas.map(function (area) {
				_this5["draw" + area.shape](_this5.scaleCoords(area.coords), area.fillColor, area.lineWidth || _this5.props.lineWidth, area.strokeColor || _this5.props.strokeColor);
			});
		}
	}, {
		key: "highlightAreas",
		value: function highlightAreas() {
			var _this6 = this;

			if (this.props.highlight) {
				var areas = this.state.map.areas.filter(function (a) {
					return a.name === _this6.props.highlight;
				});
				areas.map(function (area) {
					_this6["draw" + area.shape](_this6.scaleCoords(area.coords), area.highlightColor || _this6.props.highlightColor, area.lineWidth || _this6.props.lineWidth, area.strokeColor || _this6.props.strokeColor);
				});
			}
		}
	}, {
		...
		value: function renderAreas() {
			...
			return this.state.map.areas.map(function (area, index) {
				var scaledCoords = _this7.scaleCoords(area.coords);
				var center = _this7.computeCenter(area);
